...
try:
    ...
    PANDAS_EXPRESSIONS = OrderedDict((
                     ...
                     (b'" "',                                         b' '),
                     (b'``quick``fox',                                {'data': pandas.Series(numpy.array([qnull(QSYMBOL), numpy.bytes_('quick'), qnull(QSYMBOL), numpy.bytes_('fox')])),
                                                                      'meta': MetaData(qtype = QSYMBOL_LIST) }),
                     (b'`the`quick`brown`fox',                        {'data': pandas.Series(numpy.array([numpy.bytes_('the'), numpy.bytes_('quick'), numpy.bytes_('brown'), numpy.bytes_('fox')])),
                                                                      'meta': MetaData(qtype = QSYMBOL_LIST) }),
                     ...
                     (b'(0x01;0x02;0xff)',                            {'data': pandas.Series(numpy.array([1, 2, 0xff], dtype = numpy.uint8)),
                     ...
                     (b'flip `name`iq!(`Dent`Beeblebrox`Prefect;98 42 126)',
                                                                     {'data': pandas.DataFrame(OrderedDict((('name', pandas.Series(['Dent', 'Beeblebrox', 'Prefect'], dtype = numpy.bytes_)),
                     ...
                     (b'flip `name`iq`grade!(`Dent`Beeblebrox`Prefect;98 42 126;"a c")',
                                                                     {'data': pandas.DataFrame(OrderedDict((('name', pandas.Series(['Dent', 'Beeblebrox', 'Prefect'], dtype = numpy.bytes_)),
                                                                                                            ('iq', pandas.Series(numpy.array([98, 42, 126], dtype = numpy.int64))),
                                                                                                            ('grade', pandas.Series(['a', ' ', 'c'], dtype = str).replace(b' ', numpy.nan)),
                                                                                                             ))
                                                                                               ),
                                                                      'meta': MetaData(**{'qtype': QTABLE, 'name': QSYMBOL_LIST, 'iq': QLONG_LIST, 'grade': QSTRING}) }),
                     (b'1#([] sym:`x`x`x;str:"  a")',
                                                                     {'data': pandas.DataFrame(OrderedDict((('sym', pandas.Series(['x'], dtype = numpy.bytes_)),
                                                                                                            ('str', pandas.Series([' '], dtype = str).replace(b' ', numpy.nan)),
                                                                                                             ))
                                                                                               ),
                     ...
                     (b'-1#([] sym:`x`x`x;str:"  a")',
                                                                     {'data': pandas.DataFrame(OrderedDict((('sym', pandas.Series(['x'], dtype = numpy.bytes_)),
                                                                                                            ('str', pandas.Series(['a'], dtype = str)),
                                                                                                             ))
                                                                                               ),
                                                                      'meta': MetaData(**{'qtype': QTABLE, 'sym': QSYMBOL_LIST, 'str': QSTRING}),
                                                                      'single_char_strings': True}),
                     (b'2#([] sym:`x`x`x`x;str:"  aa")',
                                                                     {'data': pandas.DataFrame(OrderedDict((('sym', pandas.Series(['x', 'x'], dtype = numpy.bytes_)),
                                                                                                            ('str', pandas.Series([' ', ' '], dtype = str).replace(b' ', numpy.nan)),
                                                                                                             ))
                                                                                               ),
                                                                      'meta': MetaData(**{'qtype': QTABLE, 'sym': QSYMBOL_LIST, 'str': QSTRING})}),
                     (b'-2#([] sym:`x`x`x`x;str:"  aa")',
                                                                     {'data': pandas.DataFrame(OrderedDict((('sym', pandas.Series(['x', 'x'], dtype = numpy.bytes_)),
                                                                                                            ('str', pandas.Series(['a', 'a'], dtype = str).replace(b' ', numpy.nan)),
                                                                                                             ))
                                                                                               ),
                                                                      'meta': MetaData(**{'qtype': QTABLE, 'sym': QSYMBOL_LIST, 'str': QSTRING})}),
                     (b'flip `name`iq`fullname!(`Dent`Beeblebrox`Prefect;98 42 126;("Arthur Dent"; "Zaphod Beeblebrox"; "Ford Prefect"))',
                                                                     {'data': pandas.DataFrame(OrderedDict((('name', pandas.Series(['Dent', 'Beeblebrox', 'Prefect'], dtype = numpy.bytes_)),
                                                                                                            ('iq', pandas.Series(numpy.array([98, 42, 126], dtype = numpy.int64))),
                                                                                                            ('fullname', pandas.Series(["Arthur Dent", "Zaphod Beeblebrox", "Ford Prefect"], dtype = numpy.bytes_)),
                                                                                                             ))
                                                                                               ),
                     ...
                     (b'([] sc:1 2 3; nsc:(1 2; 3 4; 5 6))',          {'data': pandas.DataFrame(OrderedDict((('sc', pandas.Series(numpy.array([1, 2, 3], dtype = numpy.int64))),
                                                                      ...
                                                                      'meta': MetaData(**{'qtype': QTABLE, 'nsc': QGENERAL_LIST, 'sc': QLONG_LIST}) }),
                     (b'([] name:`symbol$(); iq:`int$())',            {'data': pandas.DataFrame(OrderedDict((('name', pandas.Series(numpy.array([], dtype = numpy.bytes_))),
                     ...
                     (b'([] pos:`d1`d2`d3;dates:(2001.01.01;2000.05.01;0Nd))',
                                                                     {'data': pandas.DataFrame(OrderedDict((('pos', pandas.Series(numpy.array(['d1', 'd2', 'd3'], dtype = numpy.bytes_))),
                     ...
                     (b'([eid:1001 1002 1003] pos:`d1`d2`d3;dates:(2001.01.01;2000.05.01;0Nd))',
                                                                     {'data': pandas.DataFrame(OrderedDict((('eid', pandas.Series(numpy.array([1001, 1002, 1003], dtype = numpy.int64))),
                                                                                                            ('pos', pandas.Series(numpy.array(['d1', 'd2', 'd3'], dtype = numpy.bytes_))),
                                                                                                            ('dates', pandas.Series(numpy.array([numpy.datetime64('2001-01-01'), numpy.datetime64('2000-05-01'), numpy.datetime64('NaT')], dtype='datetime64[D]')))))
                                                                                               ),
                                                                      'meta': MetaData(**{'qtype': QKEYED_TABLE, 'pos': QSYMBOL_LIST, 'dates': QDATE_LIST, 'eid': QLONG_LIST}),
                                                                      'index': ['eid'] }),
                     (b'([k: 1 2 3] v: `a`b`c)',
                                                                     {'data': pandas.DataFrame({'k':numpy.array([1, 2, 3], dtype = numpy.int64),'v':numpy.array(['a', 'b', 'c'], dtype = numpy.bytes_)}),
